...
// com try catch e expected.assertions(1);
...
test('Testando com async/await, rejected', async () => {
  expect.assertions(1);
  try { 
...
describe('Testando Async/Await - findAnimalsByType', () => {
  ...
  describe('Quando o tipo de animal não existe', () => {
    test('Retorna um erro', async () => {
      expect.assertions(1);
      await expect(findAnimalsByType('Lion')).rejects.toEqual(new Error('Não possui esse tipo de animal.'))
    });
  });
});
